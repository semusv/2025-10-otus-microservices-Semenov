...
import static com.microservices.warehouse.model.Reservation.SagaStep.COMPENSATION;
...
public class ReservationCompensationServiceImpl implements ReservationCompensationService {
    ...
    private CompensationResponseEvent createNewCompensatedOperation(
        ...
        // Выполняем возврат позиций
        operationReservation
                .getItems()
                .forEach(item -> catalogRepository.incrementQuantityById(
                        item.getCatalog().getId(), item.getQuantity()));
